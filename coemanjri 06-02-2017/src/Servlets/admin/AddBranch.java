...
public class AddBranch extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	try
    	{
            ...
            request.getRequestDispatcher("branchmaster.jsp").forward(request, response);
    	}
    	catch(Exception e)
    	{
    		e.printStackTrace();
    		response.sendRedirect("branchmaster.jsp?success=3");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println(bean.isBranch());
		if(session.getAttribute("cat")!=null)
		{
			if(session.getAttribute("cat").equals("Administrator")&&bean.isBranch())
			{
